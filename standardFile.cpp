...
std::vector<std::array<int, 2>> childrenPairs (int red, int blue) {
...
std::array<int, 2> bestMove (int red, int blue, bool currentNodeType, int & parentCentralizedValue) {    
    ...
    auto children = childrenPairs(red, blue);
    ...
    for (auto i : children) {
        ...
        if (childRed == 0 || childBlue == 0) { 
            ...
            bestMove(childRed, childBlue, !currentNodeType, centralizedValue);
        }
